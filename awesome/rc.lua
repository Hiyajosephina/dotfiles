...
awful.widget.watch('free', interval, function(widget, stdout)
    local _, ram_used, _ = stdout:match('Mem:%s+(%d+)%s+(%d+)%s+(%d+)')
    local ram_usage = math.floor(tonumber(ram_used) / 1024)
    widget:set_markup(markup.fontfg(beautiful.font, "#e0da37",
                                    ram_usage .. "MiB  "))
end, memory)
