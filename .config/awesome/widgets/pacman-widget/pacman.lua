...
local function worker(user_args)
    ...
    function pacman_widget.set(new_value)
        ...
        pacman_widget:get_children_by_id("icon")[1]:set_image(ICON_DIR ..
                                                                  (tonumber(
                                                                      new_value) >
                                                                      0 and
                                                                      "pacman" or
    ...
    rows:connect_signal("button::press", function(_, _, _, button)
        ...
        elseif button == 5 then
            if ptr < #rows.children and
                ((#rows.children - ptr) > _config.popup_height) then
                ptr = ptr + 1
    ...
    timer = select(2,
                   awful.widget
                       .watch([[bash -c "checkupdates 2>/dev/null"]],
                              _config.interval, function(widget, stdout)
        local upgrades_tbl = {}
        ...
        local header = wibox.widget {
            {
                ...
                {
                    markup = "<b>" ..
                        (upgrading and "Upgrading " .. #upgrades_tbl ..
                            " Packages" or
                            (#upgrades_tbl == 0 and "No" or #upgrades_tbl) ..
                            " Available Upgrades") .. "</b>",
        ...
        for k, v in ipairs(upgrades_tbl) do
            for i = 1, #rows.children do
                if v == rows.children[i].get_txt() then
                    goto continue
                end
            end
